...
def diff_map_pair(base_path, gt_path, map_name):
    ...
    for s, base_id, gt_id in sorted(diff_string_ids, key=lambda x: x[1])[:50]:  # Limit to first 50
        rows.append((map_name, "String Pool", "ID Mismatch", f"{s}", f"Base ID: {base_id}, GT ID: {gt_id}"))
    ...
    rows.append((map_name, "SidesList", "Player Count", f"Base: {len(base_sides.players)}", f"GT: {len(gt_sides.players)}"))
    ...
    for i in range(min(len(base_sides.players), len(gt_sides.players))):
        ...
        if bp_name != gp_name:
            rows.append((map_name, "SidesList", f"Player[{i}] Name", bp_name, gp_name))
        ...
        if bp_ai != gp_ai:
            rows.append((map_name, "SidesList", f"Player[{i}] aiPersonality", bp_ai, gp_ai))
        ...
        if bp_color != gp_color:
            rows.append((map_name, "SidesList", f"Player[{i}] playerColor", bp_color, gp_color))
        ...
        if bp_radar != gp_radar:
            rows.append((map_name, "SidesList", f"Player[{i}] playerRadarColor", bp_radar, gp_radar))
    
    # 4. Teams
    base_teams = base_ctx.get_asset('Teams')
    gt_teams = gt_ctx.get_asset('Teams')
    rows.append((map_name, "Teams", "Count", f"Base: {len(base_teams.teams)}", f"GT: {len(gt_teams.teams)}"))
    ...
    for tn in only_base_teams:
        rows.append((map_name, "Teams", "Only in Base", tn, ""))
    for tn in only_gt_teams:
        rows.append((map_name, "Teams", "Only in GT", tn, ""))
        ...
        if export_prop and export_prop.data:
            ...
            for bt in base_teams.teams:
                ...
                if btn == tn:
                    ...
                    if base_export != "True":
                        rows.append((map_name, "Teams", f"{tn} exportWithScript", base_export, "True"))
                    break
            if not base_has:
                rows.append((map_name, "Teams", f"{tn} exportWithScript", "N/A (team missing)", "True"))
    
    # 5. ObjectsList
    base_objects = base_ctx.get_asset('ObjectsList')
    gt_objects = gt_ctx.get_asset('ObjectsList')
    rows.append((map_name, "ObjectsList", "Count", f"Base: {len(base_objects.map_objects)}", f"GT: {len(gt_objects.map_objects)}"))
    ...
    for uid in sorted(gt_archon_set - base_archon_set):
        rows.append((map_name, "ObjectsList", "Archon Object (GT only)", uid, ""))
    ...
    for o in gt_objects.map_objects:
        ...
        if uid and 'EasterIslandHead' in uid.data:
            ...
            rows.append((map_name, "ObjectsList", f"Keepalive {uid.data}", f"owner={owner}, pos=({pos[0]:.1f},{pos[1]:.1f}), indestructible={indestructible}, enabled={enabled}", ""))
    
    # 6. PlayerScriptsList
    base_psl = base_ctx.get_asset('PlayerScriptsList')
    gt_psl = gt_ctx.get_asset('PlayerScriptsList')
    rows.append((map_name, "PlayerScriptsList", "List Count", f"Base: {len(base_psl.script_lists)}", f"GT: {len(gt_psl.script_lists)}"))
    ...
    for i in range(min(len(base_psl.script_lists), len(gt_psl.script_lists))):
        ...
        if base_scripts != gt_scripts or base_groups != gt_groups:
            rows.append((map_name, "PlayerScriptsList", f"List[{i}]", f"Base: {base_scripts} scripts, {base_groups} groups", f"GT: {gt_scripts} scripts, {gt_groups} groups"))
    
    # 7. BuildLists
    base_builds = base_ctx.get_asset('BuildLists')
    gt_builds = gt_ctx.get_asset('BuildLists')
    rows.append((map_name, "BuildLists", "Count", f"Base: {len(base_builds.build_list)}", f"GT: {len(gt_builds.build_list)}"))
    
    # 8. LibraryMapLists
    base_libs = base_ctx.get_asset('LibraryMapLists')
    gt_libs = gt_ctx.get_asset('LibraryMapLists')
    rows.append((map_name, "LibraryMapLists", "Count", f"Base: {len(base_libs.library_maps)}", f"GT: {len(gt_libs.library_maps)}"))
    
    # 9. MPPositionList
    base_mp = base_ctx.get_asset('MPPositionList')
    gt_mp = gt_ctx.get_asset('MPPositionList')
    rows.append((map_name, "MPPositionList", "Count", f"Base: {len(base_mp.positions)}", f"GT: {len(gt_mp.positions)}"))
    ...
    for i, (bp, gp) in enumerate(zip(base_mp.positions, gt_mp.positions)):
        if bp.is_human != gp.is_human or bp.is_computer != gp.is_computer or bp.team != gp.team:
            rows.append((map_name, "MPPositionList", f"Position[{i}]", f"Base: human={bp.is_human}, comp={bp.is_computer}, team={bp.team}", f"GT: human={gp.is_human}, comp={gp.is_computer}, team={gp.team}"))
    ...
    if base_cam_size != gt_cam_size:
        rows.append((map_name, "NamedCameras", "Size", f"Base: {base_cam_size} bytes", f"GT: {gt_cam_size} bytes"))
    ...
    if base_swa_count != gt_swa_count:
        rows.append((map_name, "StandingWaveAreas", "Count", f"Base: {base_swa_count}", f"GT: {gt_swa_count}"))
    ...
    if base_wi and gt_wi:
        ...
        if base_tts != gt_tts:
            rows.append((map_name, "WorldInfo", "terrainTextureStrings", base_tts[:100] if len(base_tts) > 100 else base_tts, gt_tts[:100] if len(gt_tts) > 100 else gt_tts))
    ...
    for base_asset in base_ctx.map_struct.assets:
        ...
        if gt_asset:
            ...
            if base_size != gt_size:
                rows.append((map_name, "Asset Sizes", name, f"Base: {base_size} bytes", f"GT: {gt_size} bytes (diff: {gt_size-base_size:+d})"))
...
def main():
    ...
    # Compare HF base vs HF Archon GT
    hf_base = '../RA3 Official maps/3 HF/map_mp_3_feasel3.map'
    hf_gt = '../RA3 Official maps/archon 3 player examples/[Archon]Hidden_Fortress_1.2/[Archon]Hidden_Fortress_1.2.map'
    all_rows.extend(diff_map_pair(hf_base, hf_gt, "HF"))
    
    # Compare Caldera base vs Caldera Archon GT
    caldera_base = '../RA3 Official maps/3 Caledra of Chaos/map_mp_3_feasel2.map'
    caldera_gt = '../RA3 Official maps/archon 3 player examples/[Archon]Caldera_of_Chaos_1.2/[Archon]Caldera_of_Chaos_1.2.map'
    all_rows.extend(diff_map_pair(caldera_base, caldera_gt, "Caldera"))
