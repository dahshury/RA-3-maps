...
from ..utils.refpack import RefPackCompressor
...
class Ra3MapReconstructor:
    ...
    def save_map(self, context: MapDataContext, output_path: str, compress: bool = True) -> None:
        ...
        if compress:
            # Produce a standard RA3 compressed `.map` (EAR\0 + RefPack stream),
            # matching the format used by the C# toolchain and official maps.
            compressed_data = RefPackCompressor.compress(uncompressed_data)
        else:
    ...
    def _save_map_struct(self, bw: BinaryIO, context: MapDataContext) -> None:
        ...
        # Write assets
        # Save included assets
        for asset in map_struct.assets:
            if self.config.should_save_asset(asset.get_asset_name()):
                asset.save(bw, context)
        
        # Save excluded assets as empty (if they were in the original file)
        for asset_name, excluded_asset in context.excluded_assets.items():
            if not self.config.should_save_asset(asset_name):
                # Save as empty asset
                excluded_asset.data = b""  # Ensure it's empty
                excluded_asset.save(bw, context)
