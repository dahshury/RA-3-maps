...
class Ra3MapParser:
    ...
    def __init__(self, config: Optional['ParserConfig'] = None):
        ...
        Args:
            config: ParserConfig to control what to include/exclude.
                   If None, uses default config (includes everything).
        """
        from .parser_config import ParserConfig
        self.config = config if config is not None else ParserConfig.default()
    ...
    def _do_parse_map(self, br: BinaryIO) -> MapDataContext:
        ...
        # Store config in context for use by assets
        context._parser_config = self.config
        ...
        while br.tell() < self._get_stream_size(br):
            ...
            asset_name_str = asset_name or f"Asset_{asset_id}"
            
            # Check if asset should be parsed
            if not self.config.should_parse_asset(asset_name_str):
                # Read it as default to advance stream position, but store as excluded
                default_asset = DefaultMajorAsset(asset_name_str)
                default_asset.from_stream(br, context)
                # Store as excluded asset (empty data for reconstruction)
                default_asset.data = b""  # Make it empty
                context.excluded_assets[asset_name_str] = default_asset
                continue
    ...
    def _parse_asset(self, br: BinaryIO, asset_name: Optional[str], 
        ...
        if asset_name == ASSET_HeightMapData:
            ...
            # BuildLists format varies across maps; our parser isn't fully robust yet.
            # For training/visualization we mainly need the stream to advance correctly.
            # Try to parse; if it fails, fall back to raw bytes to keep going.
            start_pos = br.tell()
            try:
