...
def _rotate_grid_xy(arr: Optional[np.ndarray], turns_cw: int) -> Optional[np.ndarray]:
    """
    Rotate a (W, H) [x, y]-indexed array CW by `turns_cw * 90°`.

    Must agree with `_rotate_xy_index`: a value at old[x, y] must end up at
    new[y, W-1-x] for turns_cw=1, etc. The previous np.rot90-based version
    silently produced a CCW rotation, so heights/passability/etc. drifted out
    of alignment with `tiles` (which uses index-based remap) and with object
    positions — this manifested as cliff walls placed off the cliffs after a
    90° rotation.
    """
    if arr is None:
        return None
...
# Two applications of _ROT_90 must equal _ROT_180. The "Except<corner>" entries
# (4/8/20/24) previously cycled CCW (4→8→20→24→4) which broke that invariant —
# correct CW cycle is 4→20→24→8→4 (ExceptTR→ExceptBR→ExceptBL→ExceptTL→ExceptTR),
# matching how the corners themselves rotate (TL→TR→BR→BL).
_ROT_180 = {
    1: 17,
    17: 1,
    2: 18,
    ...
    40: 52,
    52: 40,
    33: 49,  # Left|0x20 <-> Right|0x20
    49: 33,
    34: 50,  # Bottom|0x20 <-> Top|0x20
    50: 34,
}
...
_ROT_90 = {
    ...
    17: 2,
    2: 1,
    4: 20,    # ExceptTopRight  -> ExceptBottomRight
    20: 24,   # ExceptBottomRight -> ExceptBottomLeft
    24: 8,    # ExceptBottomLeft -> ExceptTopLeft
    8: 4,     # ExceptTopLeft   -> ExceptTopRight
    36: 52,
    52: 56,
    56: 40,
    40: 36,
    33: 50,  # Left|0x20 -> Top|0x20
    50: 49,
    49: 34,
    34: 33,
}
...
# --- Axis-flip (mirror) tables for BlendDirection ---
# axis='y'  =>  mirror x  =>  Left <-> Right (top/bottom unchanged)
_FLIP_X_MIRROR = {
    1: 17, 17: 1,    # Left <-> Right
    2: 2, 18: 18,    # Bottom / Top unchanged
    4: 8, 8: 4,      # ExceptTopRight <-> ExceptTopLeft
    20: 24, 24: 20,  # ExceptBottomLeft <-> ExceptBottomRight
    36: 40, 40: 36,  # BottomLeft <-> BottomRight
    52: 56, 56: 52,  # TopLeft <-> TopRight
    33: 49, 49: 33,  # Left|0x20 <-> Right|0x20
    34: 34, 50: 50,  # Bottom|0x20 / Top|0x20 unchanged
}

# axis='x'  =>  mirror y  =>  Top <-> Bottom (left/right unchanged)
# Note: ExceptBottomRight = 20, ExceptBottomLeft = 24 (per BlendDirection enum) —
# earlier this table had the Except-corner pairs swapped, which broke the
# flipx∘flipy = rot180 invariant.
_FLIP_Y_MIRROR = {
    1: 1, 17: 17,    # Left / Right unchanged
    2: 18, 18: 2,    # Bottom <-> Top
    4: 20, 20: 4,    # ExceptTopRight (4) <-> ExceptBottomRight (20)
    8: 24, 24: 8,    # ExceptTopLeft (8)  <-> ExceptBottomLeft  (24)
    36: 52, 52: 36,  # BottomLeft <-> TopLeft
    40: 56, 56: 40,  # BottomRight <-> TopRight
    33: 33, 49: 49,  # Left|0x20 / Right|0x20 unchanged
    34: 50, 50: 34,  # Bottom|0x20 <-> Top|0x20
}


def _flip_blend_direction_value(val: int, axis: str) -> int:
    if axis == "y":
        return _FLIP_X_MIRROR.get(val, val)
    if axis == "x":
        return _FLIP_Y_MIRROR.get(val, val)
    raise ValueError(f"axis must be 'x' or 'y', got {axis!r}")


def _flip_blend_direction(dir_in: BlendDirection, axis: str) -> BlendDirection:
    return BlendDirection(_flip_blend_direction_value(int(dir_in), axis))


def _flip_xy_index(old_x: int, old_y: int, old_w: int, old_h: int, axis: str) -> Tuple[int, int]:
    if axis == "y":  # rotate around Y axis -> mirror x
        return old_w - 1 - old_x, old_y
    if axis == "x":  # rotate around X axis -> mirror y
        return old_x, old_h - 1 - old_y
    raise ValueError(f"axis must be 'x' or 'y', got {axis!r}")
...
def _flip_angle_degrees(angle_deg: float, axis: str) -> float:
    # axis='y' (mirror x): angle -> 180 - angle
    # axis='x' (mirror y): angle -> 360 - angle (== -angle)
    if axis == "y":
        return (180.0 - angle_deg) % 360.0
    if axis == "x":
        return (360.0 - angle_deg) % 360.0
    raise ValueError(f"axis must be 'x' or 'y', got {axis!r}")
...
def rotate_context_right_angles(context, degrees: int, clockwise: bool = True) -> None:
    ...
    # Capture playable dims BEFORE mutating HeightMapData (objects/water/triggers/scripts
    # are rotated against the OLD playable extent — reading height.playable_width after
    # the in-place mutation below would silently use the NEW dims and break 90° rotations).
    ...
    if height is not None:
        old_play_w = int(height.playable_width)
        old_play_h = int(height.playable_height)
    else:
        old_play_w = old_w_full - 2 * border_tiles
        old_play_h = old_h_full - 2 * border_tiles
    ...
    if blend is not None:
        ...
        # Note: tile-value encoding is `current(x,y) + 64*tex`, but a small fraction
        # of real maps store secondary_texture_tile values that are NOT canonical at
        # the cell position (they have a non-zero residual mod 64). Preserve the
        # residual so a round-trip round-trips bit-exactly.
        for ox in range(old_w_full):
            for oy in range(old_h_full):
                nx, ny = _rotate_xy_index(ox, oy, old_w_full, old_h_full, turns_cw)

                tv_old = int(old_tiles[ox, oy])
                tex = _get_texture_from_tile(ox, oy, tv_old)
                t_residual = tv_old - _get_tile_value(ox, oy, tex)
                new_tiles[nx, ny] = np.uint16(_get_tile_value(nx, ny, tex) + t_residual)

                old_idx = int(old_blends[ox, oy])
                if old_idx > 0 and 1 <= old_idx <= len(old_info):
                    oi = old_info[old_idx - 1]
                    sec_old = int(oi.secondary_texture_tile)
                    sec_tex = _get_texture_from_tile(ox, oy, sec_old)
                    s_residual = sec_old - _get_tile_value(ox, oy, sec_tex)
                    sec_tile_new = _get_tile_value(nx, ny, sec_tex) + s_residual
                ...
                if old_idx2 > 0 and 1 <= old_idx2 <= len(old_info):
                    ...
                    sec_old2 = int(oi2.secondary_texture_tile)
                    sec_tex2 = _get_texture_from_tile(ox, oy, sec_old2)
                    s_residual2 = sec_old2 - _get_tile_value(ox, oy, sec_tex2)
                    sec_tile_new2 = _get_tile_value(nx, ny, sec_tex2) + s_residual2
...
# 3D meshes can be rotated but NOT actually mirrored — a true reflection would
# flip their handedness, which RA3's renderer can't do. For asymmetric meshes
# (cliff faces with a single textured side) we approximate the mirrored
# appearance by adding an extra rotation. Default substring -> extra-angle map
# (degrees, applied AFTER the standard mirror angle). Empirically, adding 180°
# is what a mapper does manually in WorldBuilder for cliff walls.
...
# Optional exact mirror-pair substitution: when flipping, swap object type
# `key` with `value` (and vice versa). Empty by default — image-based pair
# detection on YU cliff-wall variants did not find clear mirror pairs in the
# asset library, so we have no validated defaults. Users who discover specific
# pairs experimentally (via WB) can pass their own mapping.
DEFAULT_FLIP_MESH_SUBSTITUTION: "Dict[str, str]" = {}


def _apply_mesh_fixup_angle(angle_deg: float, type_name: str, fixup: dict) -> float:
    if not fixup:
        return angle_deg
    name_upper = type_name.upper() if type_name else ""
    extra = 0.0
    for substr, deg in fixup.items():
        if substr.upper() in name_upper:
            extra += deg
            break  # apply only the first match to keep behavior predictable
    return (angle_deg + extra) % 360.0


def flip_context_axis(context, axis: str, mesh_fixup: Optional[dict] = None,
                      mesh_substitution: Optional[dict] = None) -> None:
    """
    Mirror a map across an axis (this is a 3D-style "rotation around the X or Y axis"
    when viewing the map as a horizontal plane):

      axis='x'  -> rotate around the X axis  -> mirrors the map top<->bottom (y -> max_y - y)
      axis='y'  -> rotate around the Y axis  -> mirrors the map left<->right (x -> max_x - x)

    `mesh_fixup` is a {object_type_substring: extra_angle_deg} map that adds
    an additional rotation to objects whose `type_name` contains the substring,
    compensating for asymmetric meshes that can't be truly mirrored. Pass
    `mesh_fixup={}` to disable. None uses DEFAULT_FLIP_MESH_FIXUP.

    `mesh_substitution` is a {type_name: type_name} map; if an object's exact
    type_name is a key, it is replaced with the value (intended for true
    mirror-pair variants when known, e.g. {'YU_CLIFFWALL05': 'YU_CLIFFWALL06'}).
    The map is symmetrized at call time, so both A->B and B->A swaps work from
    a single one-way entry. Pass `mesh_substitution={}` to disable. None uses
    DEFAULT_FLIP_MESH_SUBSTITUTION.

    Map dimensions are preserved; no tile-grid transpose. We re-encode tile values
    because BlendTileData stores `(texture_index, x%8, y%8)` together (see GetTile).
    """
    if axis not in ("x", "y"):
        raise ValueError(f"axis must be 'x' or 'y', got {axis!r}")

    fixup = DEFAULT_FLIP_MESH_FIXUP if mesh_fixup is None else mesh_fixup

    sub_in = DEFAULT_FLIP_MESH_SUBSTITUTION if mesh_substitution is None else mesh_substitution
    # Symmetrize so a single A->B entry implies B->A.
    sub: dict = {}
    ...
    height: Optional[HeightMapData] = context.get_asset_by_type(HeightMapData)
    if height is not None:
        ev_raw = np.asarray(height._elevations_raw)
        ev = np.asarray(height.elevations)
        if axis == "y":  # mirror x
            height._elevations_raw = np.ascontiguousarray(ev_raw[::-1, :])
            height.elevations = np.ascontiguousarray(ev[::-1, :])
        else:  # axis == 'x', mirror y
            height._elevations_raw = np.ascontiguousarray(ev_raw[:, ::-1])
            height.elevations = np.ascontiguousarray(ev[:, ::-1])
    ...
    if blend is not None:
        ...
        new_tiles = np.zeros_like(old_tiles)
        new_blends = np.zeros_like(old_blends)
        new_se = np.zeros_like(old_se)
        ...
        for ox in range(old_w_full):
            for oy in range(old_h_full):
                nx, ny = _flip_xy_index(ox, oy, old_w_full, old_h_full, axis)
                ...
                if old_idx > 0 and 1 <= old_idx <= len(old_info):
                    ...
                    s_residual = sec_old - _get_tile_value(ox, oy, sec_tex)
                    sec_tile_new = _get_tile_value(nx, ny, sec_tex) + s_residual
                    dir_new = _flip_blend_direction(oi.blend_direction, axis)
                    new_idx = get_or_add(sec_tile_new, int(dir_new), oi.i3, oi.i4)
                    new_blends[nx, ny] = np.uint16(new_idx)
                ...
                if old_idx2 > 0 and 1 <= old_idx2 <= len(old_info):
                    ...
                    s_residual2 = sec_old2 - _get_tile_value(ox, oy, sec_tex2)
                    sec_tile_new2 = _get_tile_value(nx, ny, sec_tex2) + s_residual2
                    dir_new2 = _flip_blend_direction(oi2.blend_direction, axis)
                    new_idx2 = get_or_add(sec_tile_new2, int(dir_new2), oi2.i3, oi2.i4)
                    new_se[nx, ny] = np.uint16(new_idx2)
        ...
        def _flip_grid(arr: Optional[np.ndarray]) -> Optional[np.ndarray]:
            if arr is None:
                return None
            a = np.asarray(arr)
            return np.ascontiguousarray(a[::-1, :] if axis == "y" else a[:, ::-1])

        blend.cliff_blends = _flip_grid(blend.cliff_blends)
        blend.dynamic_shrubbery = _flip_grid(blend.dynamic_shrubbery)
        blend.passability = _flip_grid(blend.passability)
        ...
        if blend.passability is not None:
            ...
            blend.impassable = impassable
            blend.passage_width = _flip_grid(blend.passage_width)
            blend.visibility = _flip_grid(blend.visibility)
            blend.buildability = _flip_grid(blend.buildability)
            blend.tiberium_growability = _flip_grid(blend.tiberium_growability)
    ...
    border_tiles = int(getattr(context, "border", 0) or 0)
    play_w = int(getattr(height, "playable_width", old_w_full - 2 * border_tiles)) if height is not None else int(old_w_full - 2 * border_tiles)
    play_h = int(getattr(height, "playable_height", old_h_full - 2 * border_tiles)) if height is not None else int(old_h_full - 2 * border_tiles)
    if play_w <= 0 or play_h <= 0:
        play_w, play_h = old_w_full, old_h_full

    objs: Optional[ObjectsList] = context.get_asset_by_type(ObjectsList)
    if objs is not None:
        for obj in objs.map_objects:
            x, y, z = obj.position
            nx, ny = _flip_world_xy(x, y, play_w, play_h, axis)
            obj.position = (float(nx), float(ny), float(z))
            mirror_angle = _flip_angle_degrees(obj.angle, axis)
            # If the object's type has an explicit mirror-pair, swap and skip
            # the fixup-angle (the substituted variant should already render
            # correctly at the pure mirror angle).
            if obj.type_name in sub:
                obj.type_name = sub[obj.type_name]
                obj.angle = float(mirror_angle)
            else:
                obj.angle = float(_apply_mesh_fixup_angle(mirror_angle, obj.type_name, fixup))
    ...
    def _flip_points_in_place(points: List[Tuple[float, float]]) -> None:
        for i, (px, py) in enumerate(points):
            rx, ry = _flip_world_xy(px, py, play_w, play_h, axis)
            points[i] = (float(rx), float(ry))

    swa = context.get_asset_by_type(StandingWaterAreas)
    if swa is not None:
        for area in swa.water_areas:
            _flip_points_in_place(area.points)
            ...
            for script in getattr(sl, "scripts", []):
                for action in getattr(script, "script_action_on_true", []):
                    for arg in getattr(action, "arguments", []):
                        if isinstance(arg, ScriptArgument) and arg.argument_type == 16:
                            ax, ay, az = arg.position
                            rx, ry = _flip_world_xy(ax, ay, play_w, play_h, axis)
                            arg.position = (float(rx), float(ry), float(az))
                for action in getattr(script, "script_action_on_false", []):
                    for arg in getattr(action, "arguments", []):
                        if isinstance(arg, ScriptArgument) and arg.argument_type == 16:
