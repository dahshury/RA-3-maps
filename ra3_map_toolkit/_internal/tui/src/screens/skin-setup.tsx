import { useMemo, useState } from "react";
import { Box, Text, useStdin } from "ink";

import { useInput } from "@/hooks/use-input";

import { Badge } from "@/components/ui/badge";
import { Banner } from "@/components/ui/banner";
import { Divider } from "@/components/ui/divider";
import { KeyValue } from "@/components/ui/key-value";
import { StatusMessage } from "@/components/ui/status-message";
import { useTheme } from "@/components/ui/theme-provider";

import type { ScannedMap } from "./setup";

export interface SkinOptions {
  source: ScannedMap | null;
  render: boolean;
  compress: boolean;
}

export const defaultSkinOptions = (): SkinOptions => ({
  source: null,
  render: true,
  compress: true,
});

export interface SkinSetupScreenProps {
  inputDir: string;
  outputDir: string;
  maps: ScannedMap[];
  skipped: { name: string; reason: string }[];
  options: SkinOptions;
  onChange: (opts: SkinOptions) => void;
  onStart: () => void;
  onBack: () => void;
  onRescan: () => void;
  onQuit: () => void;
}

type FocusZone = "maps" | "options";

export function SkinSetupScreen({
  inputDir,
  outputDir,
  maps,
  skipped,
  options,
  onChange,
  onStart,
  onBack,
  onRescan,
  onQuit,
}: SkinSetupScreenProps) {
  const theme = useTheme();
  const [zone, setZone] = useState<FocusZone>("maps");
  const [mapCursor, setMapCursor] = useState(0);
  const [optCursor, setOptCursor] = useState(0);

  const { isRawModeSupported } = useStdin();

  const setSource = (m: ScannedMap | null) =>
    onChange({ ...options, source: m });

  const renderRow = {
    label: "Render PNG minimaps",
    value: options.render ? "ON" : "OFF",
    on: options.render,
    toggle: () => onChange({ ...options, render: !options.render }),
    hint: "Generate one PNG per variant via the toolkit's minimap_generator.",
  };
  const compressRow = {
    label: "Compress output",
    value: options.compress ? "ON" : "OFF",
    on: options.compress,
    toggle: () => onChange({ ...options, compress: !options.compress }),
    hint: "Standard for in-game maps. Turn OFF for WorldBuilder debugging.",
  };
  const optionRows = [renderRow, compressRow];

  const canStart = options.source !== null;

  const counts = useMemo(() => {
    let p2 = 0;
    let p3 = 0;
    for (const m of maps) {
      if (m.players === 3) p3++;
      else p2++;
    }
    return { p2, p3 };
  }, [maps]);

  return (
    <Box flexDirection="column">
      <Box marginTop={1}>
        <KeyValue
          items={[
            { key: "Pipeline", value: "Skin / Decompose Map (8 variants per source)" },
            { key: "Input", value: inputDir },
            { key: "Output", value: outputDir },
            {
              key: "Source",
              value: options.source ? options.source.name : "(none selected)",
            },
          ]}
        />
      </Box>

      <Box marginTop={1} flexDirection="row" gap={1}>
        <Badge variant="info">{`Found ${maps.length}`}</Badge>
        <Badge variant="success">{`2p: ${counts.p2}`}</Badge>
        <Badge variant="success">{`3p: ${counts.p3}`}</Badge>
        {skipped.length > 0 && (
          <Badge variant="warning">{`Skipped: ${skipped.length}`}</Badge>
        )}
        <Badge variant={canStart ? "info" : "warning"}>{`Outputs: ${
          canStart ? 8 : 0
        }`}</Badge>
      </Box>

      {maps.length === 0 ? (
        <Box marginTop={1}>
          <Banner variant="warning" title="No maps found">
            Place .map files (or folders containing them) in {inputDir} and press
            r to rescan.
          </Banner>
        </Box>
      ) : (
        <Box marginTop={1} flexDirection="column">
          <Box flexDirection="row" gap={1}>
            <Text bold color={zone === "maps" ? theme.colors.primary : undefined}>
              {zone === "maps" ? "▶" : " "} Pick one source map
            </Text>
            <Text dimColor>(space to select)</Text>
          </Box>
          <SourceList
            maps={maps}
            cursor={mapCursor}
            active={zone === "maps"}
            selectedPath={options.source?.path ?? options.source?.name ?? null}
          />
        </Box>
      )}

      <Box marginTop={1} flexDirection="column">
        <Box flexDirection="row" gap={1}>
          <Text bold color={zone === "options" ? theme.colors.primary : undefined}>
            {zone === "options" ? "▶" : " "} Options
          </Text>
        </Box>
        <SkinOptionList
          rows={optionRows}
          cursor={optCursor}
          active={zone === "options"}
        />
      </Box>

      <Box marginTop={1}>
        <Divider />
      </Box>

      <Footer canStart={canStart} zone={zone} />

      {!canStart && (
        <Box marginTop={1}>
          <StatusMessage variant="warning">
            Move the cursor to a map and press space to select it.
          </StatusMessage>
        </Box>
      )}

      {isRawModeSupported && (
        <SkinKeyHandler
          maps={maps}
          zone={zone}
          mapCursor={mapCursor}
          optCursor={optCursor}
          optionRows={optionRows}
          canStart={canStart}
          setZone={setZone}
          setMapCursor={setMapCursor}
          setOptCursor={setOptCursor}
          setSource={setSource}
          onStart={onStart}
          onBack={onBack}
          onRescan={onRescan}
          onQuit={onQuit}
        />
      )}
    </Box>
  );
}

function SourceList({
  maps,
  cursor,
  active,
  selectedPath,
}: {
  maps: ScannedMap[];
  cursor: number;
  active: boolean;
  selectedPath: string | null;
}) {
  const theme = useTheme();
  const visible = useMemo(() => {
    const winSize = 10;
    if (maps.length <= winSize) return { items: maps, offset: 0 };
    const half = Math.floor(winSize / 2);
    const start = Math.max(0, Math.min(maps.length - winSize, cursor - half));
    return { items: maps.slice(start, start + winSize), offset: start };
  }, [maps, cursor]);

  return (
    <Box flexDirection="column">
      {visible.items.map((m, i) => {
        const idx = visible.offset + i;
        const isCursor = active && idx === cursor;
        const isSelected = (m.path ?? m.name) === selectedPath;
        const icon = isSelected ? "◉" : "○";
        const playerColor =
          m.players === 3 ? theme.colors.warning : theme.colors.info;
        let nameColor: string | undefined;
        if (isCursor) nameColor = theme.colors.primary;
        else if (!isSelected) nameColor = theme.colors.mutedForeground;

        return (
          <Box key={m.path ?? m.name} gap={1}>
            <Text color={isCursor ? theme.colors.primary : undefined}>
              {isCursor ? "›" : " "}
            </Text>
            <Text
              color={
                isSelected ? theme.colors.success : theme.colors.mutedForeground
              }
            >
              {icon}
            </Text>
            <Text color={playerColor} bold>
              {m.players}p
            </Text>
            <Text color={nameColor} bold={isCursor}>
              {m.name}
            </Text>
          </Box>
        );
      })}
      {maps.length > visible.items.length && (
        <Text dimColor>
          {"  "}… {maps.length - visible.items.length} more (use j/k or
          PgUp/PgDn)
        </Text>
      )}
    </Box>
  );
}

function SkinOptionList({
  rows,
  cursor,
  active,
}: {
  rows: { label: string; value: string; on: boolean; hint: string }[];
  cursor: number;
  active: boolean;
}) {
  const theme = useTheme();
  return (
    <Box flexDirection="column">
      {rows.map((row, idx) => {
        const isCursor = active && idx === cursor;
        const valueColor = row.on
          ? theme.colors.success
          : theme.colors.mutedForeground;
        return (
          <Box key={row.label} flexDirection="column">
            <Box gap={1}>
              <Text color={isCursor ? theme.colors.primary : undefined}>
                {isCursor ? "›" : " "}
              </Text>
              <Text
                color={isCursor ? theme.colors.primary : undefined}
                bold={isCursor}
              >
                {row.label.padEnd(26)}
              </Text>
              <Text color={valueColor} bold>
                {row.value}
              </Text>
            </Box>
            {isCursor && (
              <Box marginLeft={4}>
                <Text dimColor>{row.hint}</Text>
              </Box>
            )}
          </Box>
        );
      })}
    </Box>
  );
}

function Footer({
  canStart,
  zone,
}: {
  canStart: boolean;
  zone: FocusZone;
}) {
  const theme = useTheme();
  const keys: { key: string; label: string }[] = [];
  if (zone === "maps") {
    keys.push(
      { key: "↑↓ / j k", label: "navigate" },
      { key: "space", label: "select source" }
    );
  } else {
    keys.push(
      { key: "↑↓", label: "navigate" },
      { key: "space", label: "toggle" }
    );
  }
  keys.push(
    { key: "tab", label: "switch panel" },
    { key: "r", label: "rescan" },
    { key: "b", label: "back" },
    { key: "enter", label: canStart ? "start" : "(pick a source)" },
    { key: "q", label: "quit" }
  );

  return (
    <Box marginTop={1} flexDirection="row" gap={2}>
      {keys.map((k) => (
        <Box key={k.key} gap={1}>
          <Text color={theme.colors.primary} bold>
            {k.key}
          </Text>
          <Text dimColor>{k.label}</Text>
        </Box>
      ))}
    </Box>
  );
}

interface SkinKeyHandlerProps {
  maps: ScannedMap[];
  zone: FocusZone;
  mapCursor: number;
  optCursor: number;
  optionRows: { label: string; value: string; on: boolean; toggle: () => void; hint: string }[];
  canStart: boolean;
  setZone: (z: FocusZone | ((prev: FocusZone) => FocusZone)) => void;
  setMapCursor: (i: number | ((prev: number) => number)) => void;
  setOptCursor: (i: number | ((prev: number) => number)) => void;
  setSource: (m: ScannedMap | null) => void;
  onStart: () => void;
  onBack: () => void;
  onRescan: () => void;
  onQuit: () => void;
}

function SkinKeyHandler({
  maps,
  zone,
  mapCursor,
  optCursor,
  optionRows,
  canStart,
  setZone,
  setMapCursor,
  setOptCursor,
  setSource,
  onStart,
  onBack,
  onRescan,
  onQuit,
}: SkinKeyHandlerProps) {
  useInput((input, key) => {
    if (key.return) {
      if (!canStart) return;
      onStart();
      return;
    }
    if (input === "q" || (key.ctrl && input === "c")) {
      onQuit();
      return;
    }
    if (input === "b") {
      onBack();
      return;
    }
    if (input === "r") {
      onRescan();
      return;
    }
    if (key.tab) {
      setZone((z) => (z === "maps" ? "options" : "maps"));
      return;
    }

    if (zone === "maps") {
      if (key.upArrow || input === "k") {
        setMapCursor((i) => Math.max(0, i - 1));
      } else if (key.downArrow || input === "j") {
        setMapCursor((i) => Math.min(maps.length - 1, i + 1));
      } else if (input === " ") {
        const m = maps[mapCursor];
        if (!m) return;
        setSource(m);
      } else if (key.pageUp || input === "g") {
        setMapCursor(0);
      } else if (key.pageDown || input === "G") {
        setMapCursor(Math.max(0, maps.length - 1));
      }
    } else {
      if (key.upArrow || input === "k") {
        setOptCursor((i) => Math.max(0, i - 1));
      } else if (key.downArrow || input === "j") {
        setOptCursor((i) => Math.min(optionRows.length - 1, i + 1));
      } else if (input === " ") {
        optionRows[optCursor]?.toggle();
      }
    }
  });
  return null;
}
