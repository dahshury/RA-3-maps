...
from map_processor.assets.scripts.script_condition import ScriptCondition
...
def _create_controller_keepalive_building(controller_number: int,
    ...
    # Common property set across templates; only a few knobs differ per scheme.
...
def _copy_paired_3p_scripts_from_template(source_ctx: MapDataContext, template_ctx: MapDataContext, num_builders: int = 3) -> None:
    ...
    **align ScriptList indices to the source map's Player_1 base index**.
    ...
    # Ensure we have enough script lists for the source player count
    sides = source_ctx.get_asset("SidesList")
    required_lists = len(sides.players) if sides else (find_first_player_index(source_ctx) + num_builders * 2)
    ...
    # Copy global list 0 (replace)
    if template_psl.script_lists:
        copied0 = copy.deepcopy(template_psl.script_lists[0])
        remap_script_list(copied0, source_ctx, player_offset=0)
        source_psl.script_lists[0] = copied0
...
def _apply_paired_3p_player_start_numbering_for_different_map(context: MapDataContext) -> None:
    ...
    target_nums = [1, 3, 5]
    for (src_n, o), tgt_n in zip(starts, target_nums):
        ...
        context.map_struct.register_string(new_uid)
...
def remap_script_condition(condition: ScriptCondition,
                           target_ctx: MapDataContext,
                           player_offset: int = 0) -> None:
    """Remap string indices in a ScriptCondition."""
    # ScriptCondition contains a ScriptContent directly (not a subclass)
    remap_script_content(condition.script_content, target_ctx, is_subclass=False)
    
    # Remap player references in condition arguments
    if player_offset != 0 and hasattr(condition.script_content, 'arguments'):
...
def calculate_controller_offset(builder_pos: Tuple[float, float, float], 
    ...
    # Map center in world coordinates (10 units per tile)
    center_x = (map_width * 10) / 2
    center_y = (map_height * 10) / 2
...
def create_multiplayer_beacons(context: MapDataContext) -> List[str]:
    ...
    # Calculate positions (corners and center)
    corner_positions = [
        (0.0, 0.0, 0.0),                                    # Bottom-left
        (float(map_width), 0.0, 0.0),                      # Bottom-right  
        (0.0, float(map_height), 0.0),                     # Top-left
        (float(map_width), float(map_height), 0.0),        # Top-right
    ]
    center_position = (float(map_width) / 2, float(map_height) / 2, 0.0)
...
def _create_multiplayer_beacon(context: MapDataContext, id_num: int, 
    ...
    # Set object fields
    beacon.type_id = context.map_struct.register_string("MultiplayerBeacon")
    beacon.type_name = "MultiplayerBeacon"
    beacon.position = position
    beacon.angle = 0.0
    beacon.road_options = 0
    ...
    properties = {
        ...
        'objectTimerHours': (AssetPropertyType.int_type, 0),
        'objectTimerMinutes': (AssetPropertyType.int_type, 0),
        'objectTimerSeconds': (AssetPropertyType.float_type, 0.0),
    }
...
def sync_assets_with_template(source_ctx: MapDataContext, 
    ...
    # Copy missing strings from template's string pool
...
def transform_to_archon(source_context: MapDataContext, 
    ...
    if paired_3p and template_context:
        # For parity, copy Players 1..6 from template and splice into the source sides list.
        t_sides = template_context.get_asset("SidesList")
        if t_sides:
            # Find insertion point: where Player_1 begins in source
            insert_at = find_first_player_index(context)
            # Keep system players before Player_1, then replace tail with template tail
            new_players = sides_list.players[:insert_at]

            # Extract template players Player_1..Player_6
            tpl_players = []
            for pl in t_sides.players:
                ...
                if pn and isinstance(pn.data, str) and pn.data.startswith("Player_"):
                    try:
                        ...
                        continue
                    if 1 <= n <= 6:
                        tpl_players.append((n, pl))
            tpl_players.sort(key=lambda x: x[0])
            for _, pl in tpl_players:
                copied = copy.deepcopy(pl)
                # Re-register property ids in source string pool
                for name, prop in copied.asset_property_collection.property_map.items():
                    prop.id = context.map_struct.register_string(name)
                new_players.append(copied)

            sides_list.players = new_players
            print("  Replaced Player_1..Player_6 in SidesList using template (paired 3p parity)")
    else:
        for i in range(num_builders):
    ...
    if paired_3p and template_context:
        # For paired 3p, we must preserve source-only system teams (e.g. teamPlyrNeutral) while
        # importing the Archon team set from the template.
        # IMPORTANT: Source teams must be inserted at their original positions to maintain
        # correct ordering (e.g. teamPlyrNeutral at index 1).
        t_teams = template_context.get_asset("Teams")
        if t_teams:
            src_existing = list(teams.teams)
            ...
            # Build a set of template team keys for lookup
            template_keys = {key_for(tm) for tm in t_teams.teams}
            
            # Find source-only teams (teams in source but not in template) with their original indices
            source_only_teams = []
            for idx, tm in enumerate(src_existing):
                k = key_for(tm)
                if k not in template_keys:
                    source_only_teams.append((idx, tm, k))
            
            # Start with template teams
            new_teams: List[Team] = []
            for tm in t_teams.teams:
                copied = copy.deepcopy(tm)
                for name, prop in copied.property_collection.property_map.items():
                    prop.id = context.map_struct.register_string(name)
                new_teams.append(copied)

            # Insert source-only teams at their original positions
            # Process in reverse order to maintain correct insertion indices
            preserved = 0
            for orig_idx, tm, k in sorted(source_only_teams, key=lambda x: x[0]):
                copied = copy.deepcopy(tm)
                for name, prop in copied.property_collection.property_map.items():
                    prop.id = context.map_struct.register_string(name)
                # Insert at original position, capped to current list size
                insert_pos = min(orig_idx, len(new_teams))
                new_teams.insert(insert_pos, copied)
                preserved += 1

            teams.teams = new_teams
            print(f"  Merged Teams from template (paired 3p): template={len(t_teams.teams)}, preserved={preserved}, total={len(teams.teams)}")
    else:
